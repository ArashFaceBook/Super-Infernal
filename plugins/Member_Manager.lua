...
--vardump(extra)
      ...
      elseif get_cmd == 'glfromall' then
        send_large_msg(receiver, 'User @'..member..' < '..member_id..' > banned from all!')
        return banall_user(member_id, chat_id)
      elseif get_cmd == 'unbfromall' then
        send_large_msg(receiver, 'User @'..member..' < '..member_id..' > unbanned from all!')
...
local function run(msg, matches)
 if matches[1]:lower() == 'id' then
    ...
    elseif matches[1]:lower() == 'id' then
      local name = user_print_name(msg.from)
      savelog(msg.to.id, name.." ["..msg.from.id.."] used /id ")
      return "Group ID f " ..string.gsub(msg.to.print_name, "_", " ").. ":\n\nID:("..msg.to.id)  
    end
  end
...
if matches[1]:lower() == 'kick' then
  ...
  if matches[1]:lower() == 'glfromall' then -- Global ban
  ...
  if matches[1]:lower() == 'ubfromall' then -- Global unban
    local user_id = matches[2]
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then 
